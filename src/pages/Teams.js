...
function Teams() {
  ...
  async function searchUser() {
    let masteryChamps = [];
    await fetch(
      `https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${message}?api_key=${key}`
    )
      .then((response) => response.json())
      .then(async (data) => {
        console.log(data);
  ...
  async function getChampions(encryptionKey) {
    await fetch(
      `https://euw1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/${encryptionKey}?api_key=${key}`
    )
      .then((response) => response.json())
      .then((response) => {
  ...
  return (
    <div className="posts-container" style={{ textAlign: "center" }}>
      ...
      <button onClick={searchUser}>Update</button>
      ...
      >
        <div className="post-title">
          {posts.name} Level: {posts.summonerLevel}
        </div>
      </div>
      <div>
        {champions.map((champion) => {
          return (
            <div style={{ padding: "30px" }}>
              {champion[0]} Level: {champion[1]} Mastery Points: {champion[2]}
            </div>
          );
        })}
      </div>
